...
- ❌ **`render.py`에 대시보드 레이아웃 추가 / 온보딩이 골격을 설정하게 만들기** — 카탈로그 10종은 **쇼케이스**이며 자동 적용되지 않습니다. render.py는 기본 테이블 하나만 굽고, 온보딩은 골격을 *설정하지 않고 가리키기만* 합니다. 레이아웃을 render로 굽는 것은 굳히기 국면의 표면 확장이라 각하합니다. (원탁 4회차, 2026-07-09)
...
> **동결선의 결(원탁 5회차, 2026-07-09).** 표면(surface)은 계속 닫되, 깊이(depth)는 엽니다. 경계 한 줄 — *"기존 스키마·기존 약속 안쪽을 채우기 = 열림 / 새 스키마·새 표면·새 골격 = 닫힘."* 예: `/note`·`/decide`(이미 있는 `notes`·`decisions` 테이블 + CLAUDE.md 가 약속한 `state=DB` 채우기)와 wiki 승격·통합 검색(CLAUDE.md 가 명문으로 약속한 `knowledge=wiki`)은 **약속 부채의 청산**이라 동결 예외입니다. 반면 새 대시보드 레이아웃을 `render.py` 로 굽는 것은 여전히 닫힘 — 대시보드 요약은 새 레이아웃이 아니라 *기존 골격의 헤더 채우기*(DB 파생·render.py 순증 +40줄 이내) 조건에서만 허용합니다.
